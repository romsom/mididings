...
template <size_t N> class MonoNoteTracker {
  public:
	...
	std::optional<NoteVelo> active_note() const {
		if (notes.empty()) {
			return {};
		} else {
			return notes.back();
...
template <size_t N> class PolyVoiceAllocator {
  public:
	...
	std::optional<size_t> find_voice(uint8_t note) const {
	...
	std::optional<size_t> note_off(uint8_t note) {
